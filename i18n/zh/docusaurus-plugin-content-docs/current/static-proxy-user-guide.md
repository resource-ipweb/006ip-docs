...
![静态代理购买 — 代理属性设置](/img/static-proxy/proxy-attribute-settings.png)
...
界面右侧的**订单详情**面板为您的实时账单核算中心。该区域会动态汇总您勾选的所有配置参数，其中 IP 购买数量输入框需要输入所需的具体 IP 数量，系统将自动校验资源库存与核算价格。在为您核减「优惠金额」后，系统将输出最终的「支付价格」，点击「立即支付」即可进入订单确认页面。
