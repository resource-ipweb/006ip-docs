---
sidebar_position: 2
sidebar_label: GeeLark 云手机配置教程
description: 在 GeeLark 云手机中配置 006IP 静态住宅代理的完整教程。
---

# GeeLark 云手机配置 006IP 静态住宅代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **GeeLark 云手机** 中完成 006IP 代理配置与连通性验证。
...
完成上述流程并获取 006IP 代理的**代理 IP 地址、端口号、用户名和密码**后，即可在 GeeLark 云手机环境中完成代理配置。
...
则将 `IP`、`PORT`、`USERNAME`、`PASSWORD` 分别对应填写到 GeeLark 的代理服务器、端口、代理账号和代理密码字段中。下面按照 GeeLark 当前界面完成云手机环境创建、代理填写、连接检测和启动。

### 1、新建云手机环境

打开 GeeLark，在左侧菜单进入 **环境管理**，点击右上角的 **新建环境**。

![在 GeeLark 环境管理中点击新建环境](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-new-profile-create.png)

进入“新建环境”页面后，选择顶部的 **云手机环境**，并使用 **单个新建**。

在 **环境设置** 中填写或选择环境信息，例如：

- **名称**：填写便于识别的环境名称
- **分组**：按需选择或新建分组
- **账号平台**：按实际用途选择
- **标签**：按需添加
- **操作系统**：根据需要选择 Android 或 iOS

![GeeLark 云手机环境设置](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-profile-settings.png)

### 2、配置 006IP 代理

向下找到 **代理设置**，将 **代理方式** 选择为 **自定义**。

本文以 SOCKS5 为例，按以下方式填写：

| GeeLark 配置项 | 006IP 对应信息 |
| --- | --- |
| 代理类型 | Socks5 |
| IP 查询渠道 | IP2Location |
...
将 006IP 中获取的代理信息分别填写到对应位置。

![在 GeeLark 中填写 006IP SOCKS5 代理信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-proxy-settings.png)

页面中的 **刷新 URL** 为选填项。如果当前使用的 006IP 代理没有对应的刷新 URL，可以保持为空；**自动刷新**也可根据实际代理使用方式设置。

### 3、检测代理连接

代理信息填写完成后，点击 **检测代理**。

检测成功后，GeeLark 会在代理设置区域显示当前代理的相关信息，例如：
...
确认检测结果与当前使用的 006IP 代理节点一致后，即可继续创建环境。

![检测 GeeLark 中的 006IP 代理连接](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-proxy-check.png)

### 4、创建并启动环境

完成代理配置和检测后，点击页面底部的 **新建**。

创建完成后，返回 **环境管理** 页面，可以看到刚刚创建的云手机环境。点击环境卡片中的 **启动**，即可启动已经配置好 006IP 代理的 GeeLark 云手机环境。

![启动已配置 006IP 代理的 GeeLark 云手机环境](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/geelark-cloudphone-start.png)
