---
sidebar_position: 4
sidebar_label: MoreLogin 云手机配置教程
description: 在 MoreLogin 云手机中配置 006IP 静态住宅代理的完整教程。
---
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **MoreLogin 云手机**中完成 006IP 代理配置、网络检测与启动。
...
配置 MoreLogin 云手机前，需要准备以下四项连接信息：
...
完成上述流程并获取 006IP 代理的**代理 IP 地址、端口号、用户名和密码**后，即可在 MoreLogin 云手机环境中完成代理配置。
...
则可将 `HOST`、`PORT`、`USERNAME`、`PASSWORD` 分别对应到 MoreLogin 的代理服务器、代理账号和代理密码字段中。下面按照 MoreLogin 当前界面完成云手机创建、代理填写、网络检测和启动。

### 1、创建云手机环境

打开 MoreLogin，在左侧进入 **环境**，切换到顶部的 **云手机**，然后点击页面中的 **创建云手机**。

![在 MoreLogin 中创建云手机](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-create-cloud-phone.png)

进入创建页面后，在 **高级创建** 中根据实际需要设置云手机环境，包括：

- **计费方式**：按分钟或按月
- **机型版本**：根据需要选择 Android 版本
- **创建环境数**：填写需要创建的环境数量
- **环境名称**：填写便于识别的名称
- **环境备注**：按需填写
- **环境分组**：按需选择

![设置 MoreLogin 云手机基础信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-cloud-phone-basic-configuration.png)

### 2、配置 006IP 代理

向下找到 **网络设置**。在 **代理方式** 中选择 **新代理**，然后填写从 006IP 获取的代理信息。
...
检测成功后，页面会显示 **“检测通过”**，并展示当前代理的出口 IP 和归属地信息。确认检测结果与当前使用的 006IP 代理节点一致。

![检测 MoreLogin 中的 006IP 代理连接](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-cloud-phone-network-detection.png)

确认配置无误后，点击底部 **确定**，完成云手机环境创建。

### 4、启动云手机

创建完成后返回 **云手机** 环境列表，可以看到新建的环境及对应的网络信息。

点击该环境右侧的 **启动**，即可打开已经配置好 006IP 代理的 MoreLogin 云手机。

![启动已配置 006IP 代理的 MoreLogin 云手机](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-cloud-phone-start.png)
