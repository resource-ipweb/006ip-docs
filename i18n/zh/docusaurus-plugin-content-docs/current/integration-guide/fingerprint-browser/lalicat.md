---
sidebar_position: 10
sidebar_label: Lalicat 指纹浏览器配置教程
description: 在 Lalicat 指纹浏览器中配置 006IP 静态住宅代理的完整教程。
---

# Lalicat 指纹浏览器配置 006IP 代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **Lalicat 指纹浏览器**中完成 006IP 代理配置与连通性验证。
...
完成上述流程并获取 006IP 代理的服务器地址、端口、用户名和密码后，即可将代理添加到 Lalicat 浏览器配置中。

### 1、新建浏览器配置

打开 **Lalicat 指纹浏览器**，在左侧进入「**浏览器列表**」，点击页面上方的「**+ 添加浏览器**」。

![Lalicat 添加浏览器](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/lalicat-1.png)

### 2、填写 006IP 代理信息

进入新建浏览器配置页面后，可以先填写显示名称、分组、浏览器版本、User-Agent 等基础信息，然后在「**基本配置**」中找到「**代理设置**」，选择与 006IP 当前代理一致的协议，例如 `HTTP`、`HTTPS` 或 `SOCKS5`。

随后依次填写：

| Lalicat 配置项 | 填写说明 |
| --- | --- |
| **IP地址** | 填写 006IP 提供的代理主机地址。 |
| **端口** | 填写对应的代理端口。 |
| **登录用户** | 填写 006IP 提供的代理账号。 |
| **登录密码** | 填写对应的代理密码。 |

如果已经复制了完整的代理信息，也可以使用右侧的「**粘贴代理信息**」功能快速录入。Lalicat 支持按页面提示的格式粘贴：
...
HOST      → Lalicat「IP地址」
PORT      → Lalicat「端口」
USERNAME  → Lalicat「登录用户」
PASSWORD  → Lalicat「登录密码」
```

![Lalicat 填写 006IP 代理信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/lalicat-2.png)

> **注意：** Lalicat 中选择的代理协议必须与 006IP 当前代理实际使用的协议一致。

### 3、检查代理连接

代理信息填写完成后，点击「**检查网络**」。

检测成功后，页面会显示绿色的检测结果，并展示国家/地区、州/省、城市等信息。

如果检测没有通过，则会显示连接失败，可以优先检查代理协议、IP 地址和端口、登录用户和登录密码，以及当前 006IP 代理是否仍然有效。

![Lalicat 检查 006IP 代理连接](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/lalicat-3.png)

确认代理检测正常后，可以继续完成其他浏览器参数设置。

### 4、保存并启动浏览器配置

最后点击页面右下角的「**保存**」，完成浏览器配置。

保存后返回「**浏览器列表**」，找到刚刚创建的配置并启动，即可通过已配置的 006IP 代理访问网络。

如需进一步确认代理是否生效，也可以在当前 Lalicat 浏览器环境中访问 `https://ipinfo.io` 或其他 IP 检测网站，检查当前显示的出口 IP 是否与 006IP 代理一致。

### 5、常见问题排查

| 现象 | 建议检查项 |
| --- | --- |
| 无法连接代理 | 检查 IP 地址和端口是否填写完整，并确认当前 006IP 代理仍然有效。 |
| 提示认证失败 | 重新核对 006IP 提供的登录用户和登录密码，避免复制时带入空格。 |
| 检查网络失败 | 检查本地网络是否能够正常连接代理服务器。 |
| 已填写账密但仍无法使用 | 确认 Lalicat 中选择的代理协议与 006IP 实际代理协议一致。 |
| 浏览器启动后显示本地 IP | 返回当前浏览器配置，确认代理信息已保存并通过「检查网络」。 |
| 检测地区与预期不一致 | 核对当前填写的 006IP 代理是否为所购买的目标国家 / 地区或城市节点。 |

> 不同 Lalicat 版本的界面布局或按钮名称可能略有差异，请以当前客户端实际界面为准。请仅将代理用于合法合规的业务场景，并遵守目标网站的服务条款及当地法律法规。
