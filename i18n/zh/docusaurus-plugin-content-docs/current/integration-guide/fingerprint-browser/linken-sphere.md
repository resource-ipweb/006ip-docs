---
sidebar_position: 11
sidebar_label: Linken Sphere 指纹浏览器配置教程
description: 在 Linken Sphere 指纹浏览器中配置 006IP 静态住宅代理的完整教程。
---

# Linken Sphere 指纹浏览器配置 006IP 代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **Linken Sphere 指纹浏览器**中完成 006IP 代理配置与连通性验证。
...
配置 Linken Sphere 前，需要导出以下四项连接信息：
...
完成上述流程并获取 006IP 代理的**代理 IP 地址、端口号、用户名和密码**后，即可在 **Linken Sphere** 中完成代理配置。
...
![创建浏览器](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-1.png)

### 2、配置 006IP 代理

在会话列表中找到刚刚创建的会话，点击最右侧的 **“⋮”。**

![配置 006IP 代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-2.png)

在展开的菜单中选择 **“Set proxy”**。

进入 **“Set proxy to”** 页面后，可以为当前会话添加或选择代理。

![配置 006IP 代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-3.png)

如果还没有添加代理，点击 **“NEW PROXY”**，将从 006IP 获取的代理信息填写进去，包括：

- **代理协议**：选择与 006IP 获取代理时一致的协议，例如 `SOCKS5` 或 HTTP/HTTPS
...
![配置 006IP 代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-4.png)

填写完成后保存代理。保存成功后，代理会出现在列表中，同时可以看到该代理检测到的国家/地区、城市以及时区等信息。

![配置 006IP 代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-5.png)

如果需要一次添加多条 006IP 代理，也可以使用右上角的 **“IMPORT FROM LIST”** 批量导入代理列表。

### 3、检查代理

在代理列表中勾选刚刚添加的 006IP 代理，然后点击底部的 **“CHECK PROXY”**。

当页面出现 **“Connection successfully validated”**，说明当前代理可以正常连接。

![检查代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-6.png)
...
浏览器启动后，该会话会通过已设置的 006IP 代理连接网络。后续如需更换代理，可以再次打开会话右侧的 **“⋮”** 菜单，进入 **“Set proxy”** 重新选择或添加代理。
