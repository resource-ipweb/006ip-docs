---
sidebar_position: 7
sidebar_label: Dolphin Anty 指纹浏览器配置教程
description: 在 Dolphin Anty 指纹浏览器中配置 006IP 静态住宅代理的完整教程。
---

# Dolphin Anty 指纹浏览器配置 006IP 代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **Dolphin Anty 指纹浏览器**中完成 006IP 代理配置与连通性验证。
...
配置 Dolphin Anty 前，需要准备以下四项连接信息：
...
完成上述流程并获取 006IP 代理的**代理主机、代理端口、代理账号和代理密码**后，即可按照下面的步骤在 **Dolphin Anty** 中完成配置。
...
![新建浏览器](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dolphinanty-1.png)

### 2、填写 006IP 代理信息

进入 **“New profile”** 页面后，在 **“General”** 页面找到 **“Proxy”** 设置区域，并选择 **“New proxy”**。

在代理设置区域，根据 006IP 提供的代理信息完成配置：

- **代理协议**：选择与 006IP 实际提供的代理协议一致的类型，例如 `HTTP`、`SOCKS4` 或 `SOCKS5`。截图示例使用 `HTTP`。

- **Proxy**：填写 006IP 提供的代理服务器信息，包括代理主机、端口、代理账号和代理密码。

- **Change IP URL**：如果当前 006IP 代理提供 IP 切换或刷新链接，可填写在此处；如果没有，则保持为空即可。

- **Proxy name**：可根据需要填写代理名称，方便后续识别和管理，也可以保持为空。

代理协议必须与 006IP 实际提供的代理协议保持一致。如果协议选择错误，即使代理主机、端口、账号和密码填写正确，也可能无法正常连接。

![填写 006IP 代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dolphinanty-2.png)

代理信息填写完成后，Dolphin Anty 会对代理进行检测。

如果代理可用，代理输入框下方会显示检测状态，并显示对应的 IP 地理位置信息，例如国家/地区、州/省、城市和时区等，说明 006IP 代理已经可以正常连接。
...
- Dolphin Anty 中选择的代理协议是否与 006IP 提供的代理协议一致；
...
![创建并启动浏览器](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dolphinanty-3.png)

返回 **“All profiles”** 页面后，找到刚刚创建的配置文件并点击 **“START”**。启动后，该浏览器环境的网络请求将通过已配置的 006IP 代理访问目标网站。
