---
sidebar_position: 9
sidebar_label: Kameleo 指纹浏览器配置教程
description: 在 Kameleo 指纹浏览器中配置 006IP 静态住宅代理的完整教程。
---

# Kameleo 指纹浏览器配置 006IP 代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **Kameleo 指纹浏览器**中完成 006IP 代理配置与连通性验证。
...
完成上述流程并获取 006IP 代理的**代理主机、代理端口、代理账号和代理密码**后，即可按照下面的步骤在 **Kameleo** 中完成配置。

### 1、新建浏览器配置文件

打开 Kameleo 客户端，在 **“Profiles”** 页面点击顶部的 **“New”**，新建一个浏览器配置文件。

![新建浏览器](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/kameleo-1.png)

进入配置页面后，可先根据实际需求设置配置文件名称、设备类型、操作系统和浏览器等基础信息。

### 2、启用代理并填写 006IP 代理信息

在 **“Proxy”** 区域选择 **“With Proxy”**，然后根据 006IP 提供的代理信息完成配置。

![启用代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/kameleo-2.png)

- **代理协议**：选择与 006IP 实际提供的代理协议一致的类型，例如 `HTTP` 或 `SOCKS5`。截图示例使用 `HTTP`。

- **IP or Hostname**：填写 006IP 提供的代理主机地址。
...
![填写 006IP 代理信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/kameleo-3.png)

代理协议必须与 006IP 实际提供的协议保持一致。如果协议选择错误，即使代理主机、端口、账号和密码填写正确，也可能无法正常建立代理连接。

### 3、测试代理连接

代理信息填写完成后，点击 **“Test Proxy”**。

Kameleo 会检测代理服务器的网络连接情况，包括代理 IPv4/IPv6 连通性、HTTPS 连通性以及代理 IP 的地理位置等信息。

如果相关检测项目显示通过，并能够正常识别代理服务器位置，说明 006IP 代理已经可以正常使用。
...
- Kameleo 中选择的代理协议是否与 006IP 提供的代理协议一致；

- 当前代理是否仍然有效；

- 本地网络是否能够正常连接代理服务器。

![测试代理连接](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/kameleo-4.png)

如果只使用 IPv4 代理，IPv6 检测未通过并不一定表示代理不可用。应重点确认 IPv4、HTTPS 连接及代理位置检测是否正常。

代理测试通过后，可以继续完成浏览器指纹等其他配置。

确认无误后，可点击 **“Save”** 保存配置文件，或点击 **“Save & Start”** 保存并直接启动浏览器环境。

浏览器启动后，该配置文件的网络请求将通过已配置的 006IP 代理访问目标网站。
