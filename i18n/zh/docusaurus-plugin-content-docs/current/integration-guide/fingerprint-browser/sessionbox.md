---
sidebar_position: 15
sidebar_label: SessionBox 指纹浏览器配置教程
description: 在 SessionBox 指纹浏览器中配置 006IP 静态住宅代理的完整教程。
---

# SessionBox 指纹浏览器配置 006IP 代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **SessionBox 指纹浏览器**中完成 006IP 代理配置与连通性验证。
...
完成上述流程并获取 006IP 代理的服务器地址、端口、用户名和密码后，即可将代理添加到 SessionBox 浏览器配置文件中。

### 1、创建浏览器配置文件

打开 **SessionBox**，在「**My Profiles**」页面点击右上角的「**Create profile**」。

![SessionBox 创建配置文件](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/sessionbox-1.png)

进入创建页面后，可以先填写当前配置文件的基础信息，例如：

- **Name**：填写配置文件名称
- **Group**：按需要选择所属分组
- **Color**：可按需要设置标识颜色

在「**Proxy**」一栏中，如果还没有添加可用代理，点击右下方的「**Manage proxies**」进入代理管理页面。

![SessionBox 进入代理管理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/sessionbox-2.png)

### 2、添加 006IP 代理

进入「**My proxies**」后，点击右上角的「**Add proxy**」。

![SessionBox 添加代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/sessionbox-3.png)

在「**Create proxy**」页面中，将上一节从 006IP 后台获取的代理信息填写到对应位置：

| SessionBox 配置项 | 填写说明 |
| --- | --- |
| **Name** | 自定义代理名称，方便后续识别。 |
| **Type** | 选择与 006IP 当前代理协议一致的类型，例如 `HTTP`。 |
| **Address** | 填写 006IP 提供的代理主机地址。 |
| **Port** | 填写对应的代理端口。 |
| **Username** | 填写 006IP 提供的代理账号。 |
| **Password** | 填写对应的代理密码。 |
...
确认信息无误后，点击右上角的「**Create**」保存代理。

![SessionBox 填写 006IP 代理信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/sessionbox-4.png)

> **注意：** SessionBox 中的代理类型必须与 006IP 获取代理时使用的协议一致。

### 3、测试代理连接

代理创建完成后，会返回「**My proxies**」列表。

找到刚刚添加的代理，点击右侧的「**⋮**」，然后选择「**Test proxy**」。

SessionBox 会对当前代理进行连接测试。如果测试通过，说明代理信息填写正确，可以继续将该代理用于浏览器配置文件。

如果测试失败，可以重点检查代理类型、Address、Port、Username 和 Password 是否与 006IP 后台提供的信息一致。

![SessionBox 测试 006IP 代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/sessionbox-5.png)

### 4、绑定代理并启动配置文件

代理测试通过后，点击「**Back**」返回配置文件创建页面。

在「**Proxy**」下拉框中选择刚刚添加并测试成功的 006IP 代理，然后继续完成其他配置。

确认无误后，点击页面右上角的「**Create**」创建配置文件。

创建完成后，在「**My Profiles**」中启动对应配置文件，即可通过已配置的 006IP 代理访问网络。

如需进一步确认代理是否生效，也可以在当前 SessionBox 浏览器环境中访问 `https://ipinfo.io` 或其他 IP 检测网站，检查当前显示的出口 IP 是否与 006IP 代理一致。

### 5、常见问题排查

| 现象 | 建议检查项 |
| --- | --- |
| Test proxy 失败 | 检查代理类型、Address 和 Port 是否填写正确，并确认当前 006IP 代理仍然有效。 |
| 提示认证失败 | 重新核对 Username 和 Password，避免复制时带入空格。 |
| 测试一直失败 | 检查本地网络是否能够正常连接代理服务器。 |
| 浏览器启动后显示本地 IP | 确认已经在配置文件的 Proxy 下拉框中选中了测试通过的 006IP 代理。 |
| 检测地区与预期不一致 | 核对当前填写的 006IP 代理是否为所购买的目标国家 / 地区或城市节点。 |

> 不同 SessionBox 版本的界面布局或按钮名称可能略有差异，请以当前客户端实际界面为准。请仅将代理用于合法合规的业务场景，并遵守目标网站的服务条款及当地法律法规。
