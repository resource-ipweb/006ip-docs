---
sidebar_position: 3
sidebar_label: 比特浏览器配置教程
description: 在比特浏览器中配置 006IP 静态住宅代理的完整教程。
---
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **比特浏览器** 中完成 006IP 代理配置与连通性验证。
...
完成上述流程并获取 006IP 代理的服务器地址、端口、用户名和密码后，即可将代理添加到比特浏览器窗口中。

### 1、创建浏览器窗口

打开 **比特浏览器**，在左侧菜单进入「**浏览器窗口**」，然后点击页面左上角的「**创建窗口**」。

![比特浏览器创建窗口](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/bit-image1.png)

### 2、填写 006IP 代理信息

进入创建窗口页面后，在代理设置区域将「**代理方式**」选择为「**自定义代理**」。

根据上一节从 006IP 后台获取的代理连接信息，依次完成以下配置。

| 比特浏览器配置项 | 填写说明 |
| --- | --- |
| **代理类型** | 选择与 006IP 当前代理对应的协议类型，例如 `HTTP`。如果获取的是 SOCKS5 代理，请选择对应的 SOCKS5 类型。 |
...
![比特浏览器填写 006IP 代理信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/bit-image2.png)

请确保代理主机、端口、账号和密码与 006IP 后台提供的信息完全一致。填写错误可能导致代理检测失败或浏览器无法正常联网。

> **注意：** 比特浏览器中选择的代理类型和 IP 协议应与 006IP 当前代理的实际配置一致。

### 3、检测代理连接

代理信息填写完成后，点击右侧的「**代理检测**」。

如果检测成功，比特浏览器会显示当前代理出口 IP，以及国家/地区、州/省、城市、时区等检测信息，说明 006IP 代理已经可以正常连接。

![比特浏览器检测 006IP 代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/bit-image3.png)

此时可核对检测到的出口 IP 是否与 006IP 后台当前使用的代理信息一致。如果出口 IP 和目标地区符合预期，即可继续完成浏览器窗口的其他配置。

### 4、保存并启动浏览器窗口

代理检测通过后，可继续根据需要完成浏览器窗口的其他配置，然后点击右下角的「**确定**」保存。

创建完成后，启动该浏览器窗口。启动后，该窗口的网络请求将通过已配置的 006IP 代理服务器访问目标网站。

如需进一步确认代理是否生效，也可以在当前比特浏览器窗口中访问 `https://ipinfo.io` 或其他 IP 检测网站，检查当前显示的出口 IP 是否与 006IP 代理一致。
...
| 现象 | 建议检查项 |
| --- | --- |
| 无法连接代理 | 检查代理主机和端口是否填写正确，并确认当前 006IP 代理仍在有效期内。 |
| 提示认证失败 | 重新核对 006IP 提供的代理账号和密码，避免复制时带入空格。 |
| 检测一直超时 | 检查本地网络是否可以正常访问代理服务器。 |
| 已填写账密但仍无法使用 | 确认代理协议与 006IP 提供的代理类型一致。 |
