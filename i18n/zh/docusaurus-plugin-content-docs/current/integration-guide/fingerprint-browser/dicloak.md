---
sidebar_position: 6
sidebar_label: DICloak 指纹浏览器配置教程
description: 在 DICloak 指纹浏览器中配置 006IP 静态住宅代理的完整教程。
---

# DICloak 指纹浏览器配置 006IP 代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **DICloak 指纹浏览器**中完成 006IP 代理配置与连通性验证。
...
完成上述流程并获取 006IP 代理的**代理 IP 地址、端口号、用户名和密码**后，即可在 **DICloak** 中完成代理配置。

### 1、创建浏览器环境

打开 DICloak 客户端，在左侧进入 **“环境管理”**，然后点击页面上方的 **“创建环境”**。

![创建浏览器](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dicloak-1.png)
...
![填写代理信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dicloak-2.png)

也可以手动填写，根据 006IP 提供的代理信息依次完成配置：

- **代理类型**：选择与 006IP 实际提供的代理协议一致的类型，例如 `HTTP`、`HTTPS` 或 `SOCKS5`。截图示例使用 `SOCKS5`。

- **IP 协议**：根据代理实际情况选择 `IPv4` 或 `IPv6`。截图示例使用 `IPv4`。

- **主机:端口**：填写 006IP 提供的代理主机地址和对应端口。
...
如果页面显示 **“连接测试成功”**，并能够正常显示代理出口 IP、国家/地区、州/省、城市和时区等信息，说明 006IP 代理已经配置成功。

![检测代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dicloak-3.png)

代理检测成功后，可以继续完成浏览器指纹、语言、时区、地理位置等其他环境配置。

确认无误后，点击页面右下角的 **“确定”** 保存浏览器环境。

创建完成后，返回 **“环境管理”** 页面并启动该环境。启动后，该浏览器环境的网络请求将通过已配置的 006IP 代理访问目标网站。
...
- DICloak 中选择的代理类型是否与 006IP 提供的代理协议一致；
