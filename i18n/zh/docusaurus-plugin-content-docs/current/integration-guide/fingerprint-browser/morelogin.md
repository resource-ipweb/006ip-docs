---
sidebar_position: 13
sidebar_label: MoreLogin 指纹浏览器配置教程
description: 在 MoreLogin 指纹浏览器中配置 006IP 静态住宅代理的完整教程。
---

# MoreLogin 指纹浏览器配置 006IP 代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **MoreLogin 指纹浏览器**中完成 006IP 代理配置与连通性验证。
...
完成上述流程并获取 006IP 代理的服务器地址、端口、用户名和密码后，即可将代理添加到 MoreLogin 浏览器环境中。

### 1、打开环境并进入代理设置

打开 **MoreLogin**，在左侧进入「**环境**」页面，并切换到「**浏览器**」列表。

![MoreLogin 浏览器环境列表](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-1.png)
...
![MoreLogin 新代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-2.png)

然后根据上一节从 006IP 后台获取的代理连接信息填写：

| MoreLogin 配置项 | 填写说明 |
| --- | --- |
| **代理类型** | 选择与 006IP 当前代理一致的协议，例如 `HTTP`、`HTTPS` 或 `SOCKS5`。 |
| **IP 查询渠道** | 可保持默认的 `IP2Location`。 |
| **代理服务器** | 填写 006IP 提供的代理主机和端口。 |
...
```text
HOST:PORT → MoreLogin「代理服务器」
USERNAME  → MoreLogin「代理账号」
PASSWORD  → MoreLogin「代理密码」
```

![MoreLogin 填写 006IP 代理信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-3.png)

> **注意：** MoreLogin 中选择的代理类型要与 006IP 获取代理时使用的协议保持一致，否则代理可能无法正常连接。
...
![MoreLogin 代理检测](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-4.png)
...
![MoreLogin 保存代理后的环境列表](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-5.png)

此时在「**代理**」一栏可以看到当前环境已经绑定了对应的代理信息和地区。

点击环境右侧的「**启动**」，打开浏览器环境。启动后，该环境会通过已配置的 006IP 代理连接网络。

如需进一步确认代理是否生效，也可以在当前 MoreLogin 浏览器环境中访问 `https://ipinfo.io` 或其他 IP 检测网站，检查当前显示的出口 IP 是否与 006IP 代理一致。

### 5、常见问题排查

| 现象 | 建议检查项 |
| --- | --- |
| 无法连接代理 | 检查代理服务器地址和端口是否正确，并确认当前 006IP 代理仍然有效。 |
| 提示认证失败 | 重新核对 006IP 提供的代理账号和密码，避免复制时带入空格。 |
| 代理检测未通过 | 检查本地网络是否能够正常连接代理服务器。 |
| 已填写账密但仍无法使用 | 确认 MoreLogin 中选择的代理类型与 006IP 实际代理协议一致。 |
