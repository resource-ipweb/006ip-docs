---
sidebar_position: 4
sidebar_label: Hubstudio 指纹浏览器配置教程
description: 在 Hubstudio 指纹浏览器中配置 006IP 静态住宅代理的完整教程。
---

# Hubstudio 指纹浏览器配置 006IP 代理教程
...
其中，**静态住宅 IP**能够提供长期固定的出口地址，更适合对网络环境连续性和连接稳定性有要求的业务。本教程将以 006IP 静态住宅代理为例，依次介绍账号注册、试用流量领取、代理购买和连接凭证导出，并演示如何在 **Hubstudio 指纹浏览器**中完成 006IP 代理配置与连通性验证。
...
完成上述流程并获取 006IP 代理的服务器地址、端口、用户名和密码后，即可将代理添加到 Hubstudio 浏览器环境中。

### 1、新建浏览器环境

打开 **Hubstudio** 客户端，在「**我的环境**」页面点击「**新建环境**」，进入浏览器环境创建页面。

![Hubstudio 新建浏览器环境](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image1.png)

根据实际使用需求填写环境名称等基础信息，然后找到「**代理信息**」配置区域。

### 2、配置 006IP 代理

在代理信息区域，将「**代理方式**」选择为「**自定义**」，然后根据上一节从 006IP 后台获取的代理连接信息完成配置。

| Hubstudio 配置项 | 填写说明 |
| --- | --- |
| **代理类型** | 选择与 006IP 当前代理对应的协议，例如 `HTTP`、`HTTPS` 或 `SOCKS5`。 |
...
![Hubstudio 填写 006IP 代理信息](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image2.png)

> **注意：** Hubstudio 中选择的代理类型必须与 006IP 实际提供的代理协议一致。如果协议类型选择错误，即使服务器地址、端口和账密填写正确，也可能无法通过连接检测。

### 3、检查代理是否可用

代理信息填写完成后，点击「**检查代理**」。

如果检测结果显示「**连接测试成功**」，并能够正常获取代理出口 IP 及对应的国家/地区等信息，说明 006IP 代理已经成功连接到当前 Hubstudio 浏览器环境。

![Hubstudio 检查 006IP 代理](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image3.png)

此时可核对检测到的出口 IP 是否与 006IP 后台当前使用的代理信息一致。如果出口 IP 和目标地区符合预期，即可继续完成浏览器环境的其他配置。

### 4、保存并启动浏览器环境

代理检测通过后，继续完成浏览器环境的其他配置，然后点击「**完成**」保存。

返回「**我的环境**」页面，找到刚刚创建的环境并点击「**打开**」。浏览器启动后，该环境的网络请求将通过已配置的 006IP 代理访问目标网站。

如需进一步确认代理是否生效，也可以在当前 Hubstudio 浏览器环境中访问 `https://ipinfo.io` 或其他 IP 检测网站，检查当前显示的出口 IP 是否与 006IP 代理一致。
...
| 已填写账密但仍无法使用 | 确认 Hubstudio 中选择的代理类型与 006IP 实际代理协议一致。 |
