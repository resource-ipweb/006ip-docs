...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **MaskFog**.
...
Before configuring MaskFog, prepare these four connection fields:
...
After completing the steps above and obtaining the 006IP proxy server address, port, username, and password, add the proxy to a MaskFog browser environment.

### 1. Create a browser environment

Open **MaskFog**, then click **New Browser** in the left sidebar.

![MaskFog create browser](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/maskfog-1.png)

On the **New Browser Environment** page, enter an environment name and configure basic settings such as the browser and User Agent as needed.

![MaskFog new browser environment](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/maskfog-2.png)

Under **Device Configuration**, set **Proxy Method** to **Custom Device**, then click **Select Device**.

### 2. Configure a 006IP proxy device

On the **Select Device** page, find an available device and click **Configure Device**.

![MaskFog configure device](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/maskfog-3.png)

On **Edit Device**, enter the proxy connection details obtained from the 006IP dashboard:

| MaskFog field | What to enter |
| --- | --- |
| **Device Name** | A custom name for easy identification, such as `proxy1`. |
| **Proxy Type** | Match the 006IP protocol, such as `HTTP`, `HTTPS`, or `SOCKS5`. |
| **Host** | The 006IP proxy server address. |
| **Port** | The corresponding proxy port. |
| **Account** | The 006IP proxy username. |
| **Password** | The corresponding proxy password. |
| **Remarks** | Optional; enter notes if needed. |

For credentials exported as `HOST:PORT:USERNAME:PASSWORD`, map the fields as follows:

```text
HOST      → MaskFog "Host"
PORT      → MaskFog "Port"
USERNAME  → MaskFog "Account"
PASSWORD  → MaskFog "Password"
```

![MaskFog enter 006IP proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/maskfog-4.png)

> **Note:** The proxy type selected in MaskFog must match the protocol used by your 006IP proxy. A mismatched protocol can prevent connections even when all credentials are correct.

### 3. Test and bind the proxy

Click **Check Network** after entering the details.

![MaskFog check network](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/maskfog-5.png)

If **Connection test successful!** appears with the egress IP, country/region, state/province, and city, the 006IP proxy is working. Click **OK** to save the device.

Back on **Select Device**, find the configured device and click **Bind** on its right.

![MaskFog bind proxy device](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/maskfog-6.png)

The bound device name now appears in the **Select Device** field.

![MaskFog bound device](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/maskfog-7.png)

### 4. Create and launch the environment

Click **OK** in the bottom-left to create the environment. On **Account Management**, find it and click **Open**. Its traffic now routes through the configured 006IP proxy.

![MaskFog launch browser environment](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/maskfog-8.png)

To verify, open `https://ipinfo.io` or another IP lookup site inside MaskFog and confirm the egress IP matches your 006IP proxy.

### 5. Troubleshooting

| Symptom | What to check |
| --- | --- |
| Cannot connect | Verify the host and port and confirm the 006IP proxy is active. |
| Authentication failed | Re-check the account and password; avoid trailing spaces. |
| Network check fails | Confirm your local network can reach the proxy server. |
| Credentials are correct but it still fails | Ensure MaskFog's proxy type matches the 006IP protocol. |
| Browser shows the local IP | Confirm the proxy device was saved and bound to this environment. |
| Region mismatch | Confirm the proxy matches the country/region or city purchased. |

> UI layout and button names may vary slightly by MaskFog version. Follow your current client UI. Use proxies only for legal/compliant scenarios and follow website terms and local laws.
