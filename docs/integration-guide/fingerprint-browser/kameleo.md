...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **Kameleo**.
...
Before configuring Kameleo, prepare these four connection fields:
...
After obtaining the 006IP proxy host, port, username, and password, follow these steps to configure **Kameleo**.

### 1. Create a browser profile

Open Kameleo and click **New** at the top of the **Profiles** page.

![Kameleo create browser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/kameleo-1.png)

Set the profile name, device type, operating system, browser, and other basic options as needed.

### 2. Enable the proxy and enter 006IP details

In the **Proxy** section, select **With Proxy**, then enter the 006IP connection details.

![Kameleo enable proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/kameleo-2.png)

- **Proxy protocol**: Match the protocol actually provided by 006IP, such as `HTTP` or `SOCKS5`. The screenshot uses `HTTP`.
- **IP or Hostname**: Enter the 006IP proxy host.
- **Port**: Enter the corresponding proxy port.
- **Username**: Enter the 006IP proxy username.
- **Password**: Enter the corresponding password.

![Kameleo enter 006IP proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/kameleo-3.png)

The selected protocol must match the actual 006IP proxy protocol. A mismatch can prevent connection even when all credentials are correct.

### 3. Test the proxy connection

Click **Test Proxy**. Kameleo checks IPv4/IPv6 connectivity, HTTPS connectivity, and the proxy IP's geolocation.

If the relevant checks pass and the proxy location is identified correctly, the 006IP proxy is ready to use.

If the test fails, check:

- The proxy host and port are correct.
- The username and password are correct.
- Kameleo's selected protocol matches the protocol provided by 006IP.
- The proxy is still active.
- Your local network can reach the proxy server.

![Kameleo test proxy connection](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/kameleo-4.png)

If you use an IPv4-only proxy, a failed IPv6 check does not necessarily mean the proxy is unavailable. Focus on the IPv4, HTTPS, and proxy location results.

After the test passes, finish the browser fingerprint settings. Click **Save** to save the profile, or **Save & Start** to save and launch it immediately.

Once launched, the profile's network requests route through the configured 006IP proxy.
