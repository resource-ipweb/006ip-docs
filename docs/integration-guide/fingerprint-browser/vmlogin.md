---
sidebar_position: 17
sidebar_label: VMLogin Fingerprint Browser Guide
description: Complete guide to configuring 006ip static residential proxies in VMLogin.
---

# VMLogin Fingerprint Browser — 006IP Proxy Guide
...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **VMLogin**.
...
Before configuring VMLogin, prepare these four connection fields:
...
After obtaining the 006IP proxy IP address, port, username, and password, configure them in **VMLogin**.

### 1. Create a browser profile

Open VMLogin and click **New Browser Profile** in the left sidebar.

![Create a browser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/vmlogin-1.png)

Under **Basic Configuration**, click **Set Proxy Server**.

![Set proxy server](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/vmlogin-2.png)

### 2. Configure the 006IP proxy

Enable **Enable Proxy Server**, then enter:

- **Proxy Type**: Match the protocol, such as `HTTP Proxy` or `SOCKS5 Proxy`.
- **IP Address**: Enter the 006IP proxy host.
- **Port**: Enter the corresponding port.
- **Login User**: Enter the proxy account.
- **Login Password**: Enter the proxy password.

![Enter proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/vmlogin-3.png)

If you copied the complete proxy string, click **Paste Proxy Information**, then verify that every field was parsed correctly. The protocol must match the one selected for your 006IP proxy.

### 3. Test the proxy

Select a test site under **Detection URL**, then click **Test Proxy**. A successful test displays the egress IP, country, city, region, and time zone.

![Test proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/vmlogin-4.png)

Click **OK** to close the result, then **Save Settings**. Complete other profile settings and click **Save Profile**. Find the environment under **Browser Profiles** and launch it to use the configured 006IP proxy.
