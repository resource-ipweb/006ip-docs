---
sidebar_position: 4
sidebar_label: Hubstudio Fingerprint Browser Guide
description: Complete guide to configuring 006ip static residential proxies in Hubstudio.
---

# Hubstudio Fingerprint Browser — 006IP Proxy Guide
...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **Hubstudio**.
...
Before configuring Hubstudio, prepare these four connection fields:
...
After completing the steps above and obtaining the 006IP proxy server address, port, username, and password, add the proxy to a Hubstudio browser environment.

### 1. Create a browser environment

Open the **Hubstudio** client, go to **My Environments**, click **New Environment**, and enter the environment creation page.

![Hubstudio create browser environment](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image1.png)

Fill in the environment name and other basic details as needed, then locate the **Proxy** section.

### 2. Configure 006IP proxy

In the proxy section, set **Proxy method** to **Custom**, then fill in the connection details exported from the 006ip dashboard:

| Hubstudio field | What to enter |
| --- | --- |
| **Proxy type** | Match the 006IP protocol (e.g. `HTTP`, `HTTPS`, or `SOCKS5`). |
...
| **Proxy protocol** | Choose based on your actual proxy configuration. |
...
```text
HOST      → Hubstudio "Proxy host"
PORT      → Hubstudio "Proxy port"
USERNAME  → Hubstudio "Proxy account"
PASSWORD  → Hubstudio "Proxy password"
```

![Hubstudio enter 006IP proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image2.png)

> **Note:** The proxy type in Hubstudio must match the protocol 006IP actually provides. An incorrect type can cause connection failures even when host, port, and credentials are correct.

### 3. Check the proxy connection

After filling in the proxy details, click **Check Proxy**.

If the result shows **Connection test successful** and displays the egress IP and country/region, the 006IP proxy is successfully linked to the Hubstudio environment.

![Hubstudio check 006IP proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/hubstudio-image3.png)

Verify that the detected egress IP matches the proxy information in your 006ip dashboard. If the IP and region look correct, continue with other environment settings as needed.

### 4. Save and launch the environment

After the proxy check succeeds, finish remaining environment settings, then click **Finish** to save.

Return to **My Environments**, find the environment you just created, and click **Open**. Once the browser starts, network traffic from that environment routes through the configured 006IP proxy.

To double-check, open `https://ipinfo.io` or another IP lookup site inside the Hubstudio browser and confirm the displayed egress IP matches your 006IP proxy.
...
| Authentication failed | Re-check the account and password; avoid trailing spaces when copying. |
| Test times out | Confirm your local network can reach the proxy server. |
| Credentials correct but still fails | Ensure the proxy type matches the 006IP proxy protocol. |
| Browser shows local IP after launch | Reopen proxy settings and confirm custom proxy is saved and passed **Check Proxy**. |
| Region mismatch | Confirm the 006IP proxy matches the country/region or city you purchased. |

> UI layout and button names may vary slightly by Hubstudio version. Follow your current client UI. Use proxies only for legal/compliant scenarios and follow website terms and local laws.
