...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **Dolphin Anty**.
...
Before configuring Dolphin Anty, prepare these four connection fields:
...
After obtaining the 006IP proxy host, port, username, and password, follow these steps to configure **Dolphin Anty**.

### 1. Create a browser profile

Open Dolphin Anty and click the **+** button at the top of the **All profiles** page.

![Dolphin Anty create browser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dolphinanty-1.png)

### 2. Enter 006IP proxy details

On the **New profile** page, find **Proxy** under **General**, then select **New proxy**.

Configure the following fields:

- **Proxy protocol**: Match the protocol actually provided by 006IP, such as `HTTP`, `SOCKS4`, or `SOCKS5`. The screenshot uses `HTTP`.
- **Proxy**: Enter the 006IP proxy host, port, username, and password.
- **Change IP URL**: Enter an IP rotation or refresh URL if 006IP provides one; otherwise leave this blank.
- **Proxy name**: Optionally enter a name for easier identification and management.

The selected protocol must match the 006IP proxy protocol. A mismatch can prevent connection even if the host, port, username, and password are correct.

![Dolphin Anty enter 006IP proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dolphinanty-2.png)

Dolphin Anty tests the proxy after you enter its details. If the proxy is available, a status appears below the input along with location information such as country/region, state/province, city, and time zone.

If the test fails, check:

- The proxy host and port are correct.
- The username and password are correct.
- The selected Dolphin Anty protocol matches the protocol provided by 006IP.
- The proxy is still active.
- Your local network can reach the proxy server.

### 3. Create and launch the browser profile

After the proxy passes the test, finish configuring the browser fingerprint, platform, User-Agent, and other settings.

Click **CREATE PROFILE** in the bottom-right corner.

![Dolphin Anty create and launch profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dolphinanty-3.png)

Return to **All profiles**, find the profile, and click **START**. Its network requests will route through the configured 006IP proxy.
