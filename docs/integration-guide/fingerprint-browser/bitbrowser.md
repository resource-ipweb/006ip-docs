...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **BitBrowser**.
...
Before configuring BitBrowser, prepare these four connection fields:
...
After completing the steps above and obtaining the 006IP proxy server address, port, username, and password, add the proxy to a BitBrowser window.

### 1. Create a browser window

Open **BitBrowser**, go to **Browser Windows** in the left menu, then click **Create Window** in the top-left corner.

![BitBrowser create window](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/bit-image1.png)

### 2. Enter 006IP proxy details

On the create window page, set **Proxy method** to **Custom Proxy** in the proxy settings area, then fill in the connection details exported from the 006ip dashboard:

| BitBrowser field | What to enter |
| --- | --- |
| **Proxy type** | Match the 006IP protocol (e.g. `HTTP`; use SOCKS5 if that is what you received). |
| **IP protocol** | Choose `IPv4` or `IPv6` based on your proxy. |
...
```text
HOST      → BitBrowser "Proxy host"
PORT      → BitBrowser "Proxy port"
USERNAME  → BitBrowser "Proxy account"
PASSWORD  → BitBrowser "Proxy password"
```

![BitBrowser enter 006IP proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/bit-image2.png)

Ensure host, port, account, and password exactly match the 006ip dashboard. Incorrect values can cause proxy test failures or prevent the browser from connecting.

> **Note:** The proxy type and IP protocol in BitBrowser must match your 006IP proxy configuration.

### 3. Test the proxy connection

After filling in the proxy details, click **Proxy Test** on the right.

If the test succeeds, BitBrowser displays the egress IP along with country/region, state/province, city, and timezone—confirming the 006IP proxy is connected.

![BitBrowser test 006IP proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/bit-image3.png)

Verify that the detected egress IP matches the proxy information in your 006ip dashboard. If the IP and region look correct, continue with other window settings as needed.

### 4. Save and launch the browser window

After the proxy test succeeds, finish any remaining window settings, then click **OK** in the bottom-right corner to save.

Launch the browser window. Once started, network traffic from that window routes through the configured 006IP proxy.

To double-check, open `https://ipinfo.io` or another IP lookup site inside the BitBrowser window and confirm the displayed egress IP matches your 006IP proxy.

### 5. Troubleshooting

If the proxy test fails in BitBrowser, check the following:

| Symptom | What to check |
| --- | --- |
| Cannot connect | Verify host and port, and confirm the 006IP proxy is still valid. |
...
| Credentials correct but still fails | Ensure the proxy protocol matches the 006IP proxy type. |
| Browser shows local IP after launch | Reopen proxy settings and confirm custom proxy is saved and passed **Proxy Test**. |
| Region mismatch | Confirm the 006IP proxy matches the country/region or city you purchased. |

> UI layout and button names may vary slightly by BitBrowser version. Follow your current client UI. Use proxies only for legal/compliant scenarios and follow website terms and local laws.
