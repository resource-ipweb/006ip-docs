---
sidebar_position: 11
sidebar_label: Linken Sphere Fingerprint Browser Guide
description: Complete guide to configuring 006ip static residential proxies in Linken Sphere.
---

# Linken Sphere Fingerprint Browser — 006IP Proxy Guide
...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **Linken Sphere**.
...
Before configuring Linken Sphere, prepare these four connection fields:
...
## Configure Linken Sphere

After obtaining the 006IP proxy IP address, port, username, and password, configure it in **Linken Sphere**.

### 1. Create a browser session

Open Linken Sphere and click **NEW SESSION** in the top-left corner. The new session appears in the current Desktop's session list.

![Linken Sphere create browser session](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-1.png)

### 2. Configure the 006IP proxy

Find the new session and click **⋮** on the far right.

![Linken Sphere open session menu](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-2.png)

Select **Set proxy** from the menu. On the **Set proxy to** page, you can add or select a proxy for this session.

![Linken Sphere set proxy page](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-3.png)

If no proxy has been added, click **NEW PROXY** and enter:

- **Proxy protocol**: Match the 006IP protocol, such as `SOCKS5` or `HTTP/HTTPS`.
...
![Linken Sphere enter 006IP proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-4.png)

Save the proxy. It then appears in the list with its detected country/region, city, time zone, and other information.

![Linken Sphere saved proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-5.png)

To add multiple 006IP proxies at once, use **IMPORT FROM LIST** in the top-right corner.

### 3. Check and assign the proxy

Select the newly added 006IP proxy in the list, then click **CHECK PROXY** at the bottom.

When **Connection successfully validated** appears, the proxy can connect normally.

![Linken Sphere check proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/linkensphere-6.png)

Keep the proxy selected and click **SET** to bind it to the current session.

Return to the session list and verify the proxy under **Proxy**, then click the start button on the left of the session.

Once launched, the session connects through the configured 006IP proxy. To change it later, open the session's **⋮** menu and select **Set proxy** again.
