...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **MostLogin**.
...
Before configuring MostLogin, prepare these four connection fields:
...
After obtaining the 006IP proxy host, port, account, and password, configure them in **MostLogin** as follows.

### 1. Create a browser window

Open MostLogin and click **Create Window** on the **Windows** page.

![Create a browser window](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/mostlogin1.png)

### 2. Enter 006IP proxy details

On the create window page, select the **Proxy** tab and choose **Basic** as the proxy type. Enter:

- **Proxy Protocol**: Match the 006IP protocol, such as `HTTP`, `HTTPS`, or `SOCKS5`; the screenshot uses `HTTPS`.
- **Host:Port**: Enter the proxy server address and port.
- **Account**: Enter the 006IP proxy account.
- **Password**: Enter the corresponding password.
- **Proxy Rotation URL**: Enter a rotation or refresh URL if provided; otherwise leave blank.

![Enter 006IP proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/mostlogin2.png)

> The proxy protocol must match the protocol provided by 006IP. A mismatch can prevent a connection even when the other details are correct.

### 3. Check the proxy server IP

Click **Check Proxy Server IP**. If successful, MostLogin displays the egress IP, country/region, state/province, city, coordinates, time zone, and postal code.

![Check proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/mostlogin-3.png)

Complete any other window settings, then click **Confirm** in the bottom-right. Return to the window list and launch the window; its requests now route through the configured 006IP proxy.
