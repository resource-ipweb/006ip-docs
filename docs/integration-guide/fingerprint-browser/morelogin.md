...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **MoreLogin**.
...
Before configuring MoreLogin, prepare these four connection fields:
...
After obtaining the 006IP proxy server address, port, username, and password, add the proxy to a MoreLogin browser environment.

### 1. Open an environment and its proxy settings

Open **MoreLogin**, go to **Environment** in the left sidebar, and switch to the **Browser** list.

![MoreLogin browser environment list](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-1.png)

Find the environment to configure and click the edit icon in the **Proxy** column to open **Modify Proxy**. If no environment exists, click **New Environment** in the top-right first.

### 2. Add a 006IP proxy

Select **New Proxy** in the **Modify Proxy** window.

![MoreLogin new proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-2.png)

Enter the details obtained from the 006IP dashboard:

| MoreLogin field | What to enter |
| --- | --- |
| **Proxy Type** | Match the 006IP protocol, such as `HTTP`, `HTTPS`, or `SOCKS5`. |
| **IP Lookup Channel** | Keep the default `IP2Location` if desired. |
| **Proxy Server** | The 006IP proxy host and port. |
| **Proxy Account** | The 006IP proxy username. |
| **Proxy Password** | The corresponding password. |
| **Refresh URL** | Enter a refresh link if provided; otherwise leave blank. |

Map exported credentials as follows:
...
![MoreLogin enter 006IP proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-3.png)

> **Note:** MoreLogin's proxy type must match the protocol selected for the 006IP proxy, or the connection may fail.

### 3. Test the proxy

Click **Proxy Test** at the bottom. A successful test shows **Test Passed**, the egress IP, and its location.

![MoreLogin proxy test](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-4.png)

Click **OK** in the bottom-right to save.

### 4. Save and launch the environment

Return to the MoreLogin environment list.

![MoreLogin environment list after saving proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/morelogin-5.png)

The **Proxy** column displays the bound proxy and region. Click **Start** to launch the environment; its traffic routes through the configured 006IP proxy. Verify it at `https://ipinfo.io` if needed.

### 5. Troubleshooting

| Symptom | What to check |
| --- | --- |
| Cannot connect | Verify the proxy server and port and confirm the 006IP proxy is active. |
| Authentication failed | Re-check the proxy account and password; avoid trailing spaces. |
| Proxy test fails | Confirm your local network can reach the proxy server. |
| Credentials are correct but it still fails | Ensure MoreLogin's proxy type matches the 006IP protocol. |
| Browser shows the local IP | Confirm the proxy was saved and bound to this environment. |
| Region mismatch | Confirm the proxy matches the country/region or city purchased. |

> UI layout and button names may vary slightly by MoreLogin version. Follow your current client UI. Use proxies only for legal/compliant scenarios and follow website terms and local laws.
