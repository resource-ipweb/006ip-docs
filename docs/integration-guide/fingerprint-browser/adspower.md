...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **AdsPower**.
...
Before configuring AdsPower, prepare these four connection fields:
...
After completing the steps above and obtaining the 006IP proxy server address, port, username, and password, add the proxy to an AdsPower browser profile.

### 1. Create a browser profile

Open the **AdsPower** client, then click **New Browser** in the top-left corner.

![AdsPower create browser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/adspower-image1.png)

### 2. Enter 006IP proxy details

On the new browser page, find the proxy section, set **Proxy method** to **Custom**, then fill in the connection details exported from the 006ip dashboard:

| AdsPower field | What to enter |
| --- | --- |
| **Proxy type** | Match the 006IP protocol (e.g. `HTTP`, `HTTPS`, or `Socks5`). |
| **IP lookup channel** | Keep the default `IP2Location` to detect egress IP and location. |
| **Host:Port** | Enter the 006IP proxy server address and port. |
| **Proxy account** | Enter the 006IP proxy username. |
| **Proxy password** | Enter the matching proxy password. |
| **Refresh URL** | Fill in if 006IP provides a refresh link; otherwise leave blank. |
...
HOST      → AdsPower "Host"
PORT      → AdsPower "Port"
USERNAME  → AdsPower "Proxy account"
PASSWORD  → AdsPower "Proxy password"
```

![AdsPower enter 006IP proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/adspower-image2.png)

> **Note:** The proxy type in AdsPower must match the protocol 006IP actually provides. An incorrect type can cause connection failures even when host, port, and credentials are correct.

### 3. Check the proxy connection

After filling in the proxy details, click **Check Proxy**.

![AdsPower check proxy connection](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/adspower-image3.png)

If the page shows **Connection test successful** and displays the egress IP, country/region, and city, the 006IP proxy is successfully linked to the AdsPower profile.

Verify that the detected egress IP matches the proxy information in your 006ip dashboard. If the IP and region look correct, continue with fingerprint and platform settings as needed.

### 4. Save and launch the profile

After the proxy check succeeds, click **OK** at the bottom of the page to save the profile.

Return to **Profiles**, find the profile you just created, and open it. Once started, network traffic from that environment routes through the configured 006IP proxy.

To double-check, open `https://ipinfo.io` or another IP lookup site inside the AdsPower browser and confirm the displayed egress IP matches your 006IP proxy.

### 5. Troubleshooting

If the proxy check fails in AdsPower, check the following:
...
| Browser shows local IP after launch | Reopen proxy settings and confirm the proxy is saved and passed **Check Proxy**. |
| Region mismatch | Confirm the 006IP proxy matches the country/region or city you purchased. |

> UI layout and button names may vary slightly by AdsPower version. Follow your current client UI. Use proxies only for legal/compliant scenarios and follow website terms and local laws.
