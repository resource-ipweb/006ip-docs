...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **DICloak**.
...
Before configuring DICloak, prepare these four connection fields:
...
After obtaining the 006IP proxy IP address, port, username, and password, configure the proxy in **DICloak**.

### 1. Create a browser profile

Open DICloak, go to **Environment Management** in the left sidebar, then click **Create Environment** at the top.

![DICloak create browser profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dicloak-1.png)

### 2. Enter 006IP proxy details

On the environment creation page, select **Custom Proxy** in the **Proxy Settings** section.

If you have the complete proxy string, paste it into **Quick Input** and click **Parse**. DICloak will identify and populate the fields automatically.

![DICloak enter proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dicloak-2.png)

You can also enter the details manually:

- **Proxy Type**: Select the protocol actually provided by 006IP, such as `HTTP`, `HTTPS`, or `SOCKS5`. The screenshot uses `SOCKS5`.
- **IP Protocol**: Select `IPv4` or `IPv6` to match the proxy. The screenshot uses `IPv4`.
- **Host:Port**: Enter the 006IP proxy host and port.
- **Proxy Account**: Enter the 006IP proxy username.
- **Proxy Password**: Enter the corresponding password.
- **IP Lookup Channel**: Keep DICloak's default setting to detect the egress IP and location.

### 3. Test and save the proxy

After entering the details, click **Test Proxy**.

If DICloak displays **Connection test successful** and shows the egress IP, country/region, state/province, city, and time zone, the 006IP proxy is configured correctly.

![DICloak test proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/dicloak-3.png)

After a successful test, finish configuring the fingerprint, language, time zone, geolocation, and other environment settings. Click **OK** in the bottom-right corner to save.

Return to **Environment Management** and launch the environment. Its network requests will now route through the configured 006IP proxy.

If the test fails, check:

- The proxy host and port are correct.
- The proxy username and password are correct.
- The selected DICloak proxy type matches the protocol provided by 006IP.
- The IPv4/IPv6 option matches the proxy.
