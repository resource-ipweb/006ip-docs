...
**Static residential IPs** offer long-lived fixed egress addresses and are a better fit when you need continuity and connection stability. This guide walks through registration, trial traffic, purchasing, exporting credentials, and configuring 006IP proxies in **Undetectable**.
...
Before configuring Undetectable, prepare these four connection fields:
...
After obtaining the 006IP proxy server address, port, username, and password, add it to an Undetectable user profile.

### 1. Create a user profile

Open **Undetectable**, go to **User Profiles** in the left sidebar, and click **+ New User Profile**.

![Undetectable create user profile](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/undetectable-1.png)

### 2. Add a 006IP proxy

Enter a profile name, click the proxy settings icon, and select **New Proxy** from the **Proxy** dropdown. Choose the same protocol used by the 006IP proxy, such as `HTTP`, then enter:

```text
Host:Port:Login:Password
```

![Undetectable enter 006IP proxy details](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/undetectable-2.png)

If provided, enter an IP-change URL under **IP Change Link (optional)**. You may also assign an optional proxy name.

> **Note:** The protocol selected in Undetectable must match the protocol used by the 006IP proxy.

### 3. Check the proxy connection

Click the **double-arrow button** beside the input. A successful check displays the egress IP, country/region, and city and updates the profile details on the right.

![Undetectable test 006IP proxy](https://cdn.006ip.com/docs/img/static-proxy/config-tutorial/undetectable-3.png)

If the detected location matches your 006IP proxy and no connection error appears, the proxy is ready.

### 4. Save and launch the user profile

Configure Cookies, fingerprint, and other settings as needed, then click **Save**. To launch immediately, click **Open** instead. Verify the egress IP at `https://ipinfo.io` if needed.

### 5. Troubleshooting

| Symptom | What to check |
| --- | --- |
| Proxy test fails | Verify the protocol and complete `Host:Port:Login:Password` value. |
...
| Browser shows the local IP | Confirm the 006IP proxy was saved and passed the test. |
| Region mismatch | Confirm the proxy matches the country/region or city purchased. |

> UI layout and button names may vary slightly by Undetectable version. Follow your current client UI. Use proxies only for legal/compliant scenarios and follow website terms and local laws.
